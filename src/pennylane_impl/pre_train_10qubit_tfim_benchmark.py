...
n_samples = 7500  # Number of data samples to generate
...
def load_dataset(file_path, J_, h_, idea_):
    if idea_:
        file_name = file_path + "data_ideal.json"
        with open(file_name, 'r') as file:
            data = json.load(file)
    else:
        file_name = file_path + "data_shadow.json"
        with open(file_name, 'r') as file:
            data = json.load(file)
    return data
...
dataset_file_path = os.path.join(script_dir, '..', '..', 'results', 'data', 'pennylane_data', '10b', '')
...
models_dir = os.path.join(script_dir, '..', '..', 'results', 'data', 'pennylane_data', '10b', 'models')
results_file_path = os.path.join(models_dir, 'ML_fitting_res_Ideal.json')
idea_setting = True
...
for j_ in J_list:
    for h_ in h_list:
        print(f"J={j_}, h={h_}")
        model_file_path = os.path.join(models_dir, f'ML_fitting_model_Ideal_J={j_}.joblib')
...
input_dim = n_layers * (2*n_qubits -1)
Lambda = 2  # Maximum Hamming weight, hyper-parameter
results_file_path = os.path.join(models_dir, 'ML_fitting_res_Shadow.json')
idea_setting = False
...
for j_ in J_list:
    for h_ in h_list:
        print(f"J={j_}, h={h_}")
        model_file_path = os.path.join(models_dir, f'ML_fitting_model_Shadow_J={j_}.joblib')
        trained_model = ML_predictor(j_, h_, idea_setting, num_train, num_test, Lambda, results_file_path, model_file_path)
...
for j_ in J_list:
    for h_ in h_list:
        ...
        for iter_ in range(repeat_times):
            ...
            results_file_path = os.path.join(models_dir, 'Adam_Optimization_Results.json')
            ...
            Inter_valide_output_file = os.path.join(models_dir, f'Adam_Shadow_J={j_}Repeat{iter_}.json')
            vqe_energy_history.append({
                "iteration": 0,
